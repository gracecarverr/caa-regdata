...
> (**$206.0M** of **$4.483B**), 4.9% major/synmin, 11.6% electric (`summary_penalty.csv`: `dup_total`/`dup_share`).
> These dollars come from very few rows — **284** duplicate penalty rows (formal `dup>0` with penalty>0) in
> universe, **246** major/synmin, **20** electric (~0.55% of penalty-bearing formal rows) — but high-value
> ones (electric's 20 rows ≈ $2.6M each), so the dollar share dwarfs the row share. All are re-entries under
> the same `ENF_IDENTIFIER`, **none byte-identical** (`dup_exact=0` for formal).
...
| **W5** | **`BEGIN_DATE` ignored** for program history; snapshot **presence** (row exists & status active under the **N11 program-specific rule** — `OPR/TMP/SEA`, plus `PLN/CNS` for NSR/PSD) is the truth. Program covered = the **8 spine groups** only (`prog_*_active`). | Trust `BEGIN_DATE`; build all program codes. | `BEGIN_DATE` is 100% populated 2021+ but unreliable (junk years, no close date). Starting with the 8 already-flagged groups keeps the panel aligned with the static `prog_*` profile (F6). |
