...
| **O3** | **`operating` carried unchanged from the cleaning layer** — whitelist flag (1 iff code ∈ {OPR,TMP,SEA}). The 8 `PROG_*_ACTIVE` use the cleaning layer's **blacklist** (active unless CLS). | Re-derive here. | The two rules answer different questions and won't always agree (N11); kept faithful to source. Build asserts `operating == whitelist(op_status_code)` as a tripwire. |
