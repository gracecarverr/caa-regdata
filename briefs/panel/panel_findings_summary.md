...
| C3 | **Unplaceable / bad coordinates** | 15,730 no coordinate; **1,699 gross coord errors** (>5 km, up to ~1,970 km); 4,069 county names unresolvable (mostly CT) | No coordinate → no county. Gross errors misplace facilities across state lines. |
...
| C7 | **Major emissions cert under-coverage** | 72.5% coverage in 2025 (9,428 of 13,012 operating), declining from 77.5% in 2015; 3,584 major operating facs have no reported cert | Title V requires annual certs for majors. Gap is **reporting lag** — 2025 certs sparse in ICIS. Practical: don't assume a cert per major per year; use `any_certs` flag. |
