...
> ⚠ **Code cross-references predate the current layout** (see the same note in
> `panel_construction_decisions.md`). The open questions themselves stand; cited paths are being reconciled
> against the current `code/` tree.
...
- **Interim proxies (until the Census merge):** (a) activity-based — active in year *t* (⚠ circular if activity is also an outcome); (b) **emissions-reporting presence** — we already hold `POLL_RPT_COMBINED_EMISSIONS`, a semi-independent operating signal; (c) for electric specifically, **EIA-860** online/retirement dates — mergeable *now* via an ORISPL crosswalk; (d) **AFS historical compliance status** (pre-2015, explicit shut-down code) — gated on an AFS↔ICIS crosswalk investigated in `../datasets/afs_crosswalk_feasibility.md`: works (74.5% coverage) for the specific pre-2015-begin-year population, but with severe, likely-structural state-level variance (sub-2% in IL/OH/SC to ~100% in CO/KS/VA) that any use would need to carry forward.
...
- **Why it matters:** The choice *materially* moves yearly counts — e.g. 2005 shows **486 FRV-only vs 1,957 coalesced** `[C1]`. And the timing fields are sparse (`HPV_DAYZERO` 56% missing, `DSCV` 62%) `[E3]`.
...
**D-C5 · Should `prog_*_active` match the facility `operating` status convention?** `[N11]` — **RESOLVED 2026-07-18 (option c, program-specific).**
- **Question:** `prog_X_active` (19_) was a *blacklist* — active for every status except `CLS`, so `PLN` (planned) and `CNS` (under construction) counted as active. The facility `operating` flag (17_) is a *whitelist* — `1` only for `OPR/TMP/SEA`. So a facility in `PLN` read `operating = 0` but `prog_X_active = 1` the same year. Magnitude of the divergent statuses: `PLN` 22,340 + `CNS` 13,910 + `NER/NED/NES/LDF` 1,717 in-group program rows (~0.98% of kept). The NA→active branch is vacuous (0 rows).
- **Options considered:** (a) keep the blacklist (read as *subject-to/enrolled under*); (b) switch to the `17_` whitelist (`OPR/TMP/SEA`) so a program is active only while operating; **(c) program-specific** — the preconstruction permits **NSR & PSD** are active for `{OPR,TMP,SEA,PLN,CNS}` (they attach *before* a source runs), while the six operating programs (`sip/titlev/nsps/mact/neshap/fesop`) use the `{OPR,TMP,SEA}` whitelist. `CLS` + `NER/NED/NES/LDF` + missing → inactive for all.
- **Resolution:** **(c)** — a strict whitelist would wrongly zero NSR/PSD exactly during construction, when those permits bind; the blacklist over-counted the operating programs. `19_wayback_program_status.R` now implements (c) (filter is group-aware). Effect vs the blacklist (facility-year flags flipped `1→0`, no `0→1`): sip 14,987, mact 7,344, nsps 6,650, titlev 2,692, neshap 2,210, fesop 549; nsr 29, psd 5 (nsr/psd retain PLN/CNS). See N11 in `panel_construction_decisions.md`.
...
| **G Sample/coverage** | | G1 AFS→FRS 30% unlinked · G2 majors 16% dormant | |
