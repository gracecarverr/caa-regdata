...
|---|---|---|---|
| **R1** | **Universe = every `PGM_SYS_ID` in `ICIS-AIR_FACILITIES` (279,665 as of 2026-07-27), NO ever-active screen.** | Keep only facilities with ≥1 event in-window (the panel-layer "ever-active" universe, ~134k). | Retains the never-inspected population (mostly operating minor sources). **52.2% of facilities have zero events in-window** (145,912 of 279,665) and sit all-`NA` here until dataset 1 supplies operating status. Consequence: only **12.6%** of the rectangle is observed — the dataset is close to unusable standalone by design, which is why ds 0 and ds 1 must be documented as a pair. |
...
**Shape (as of 2026-07-27; drifts with each live ICIS-AIR refresh — see EM2):**
```
5,872,965 rows | 21 cols | 279,665 facilities × 21 years | 2,470,741 wayback-observed facility-years (42.1%)
```
...
is **~80%** of facility-years (was ~88%).
...
| **H5** | **Facility-year mapping rule = R2** (interval overlap; open/bad-order conservatively closed on Dec-31 of the day-zero year). Feeds `hpv_active`. | R1 day-zero-year-only; R3 extend open spells to window end; R4 union>30-days. | Diagnostic 09 quantified each lever over 2005–2025 (levers not independently re-verified in the 2026-07-27 refresh pass — see H7 for the current post-screen total): **R1→R2 (duration) +19,378 fac-yrs** (day-zero-year-only *halves* coverage — rejected); **R2→R3 (extend open spells) +11,546**, concentrated in recent years (an assumption about missing resolution dates — rejected as too strong); **R2→R4 (30-day threshold) −1,952** (minor). R2 chosen: faithful to spell duration without manufacturing an open-ended tail. *(User decision, 2026-07-20.)* |
...
| # | Decision | Alternative not taken | Why / data fact |
|---|---|---|---|
| **P1** | **Grain = one row per formal-action record; ALL rows kept, `dup>0` flagged not dropped.** | Dedup to distinct actions. | Layer convention (CC9/R3). As of 2026-07-27: 823 dup rows (was 857); event-distinct = 105,123 (was 104,799). |
...
| **P5** | **Multi-facility settlement structure EXPOSED, not resolved** — `ENF_IDENTIFIER` (settlement key), `N_SETTLEMENT_FACILITIES`, `IS_MULTI_FACILITY`; per-row penalty left faithful. | Deduplicate broadcast penalties, or split them across co-defendants. | ⚠ As of 2026-07-27: **571 settlements** span >1 facility (was 588; up to **117** co-defendants unchanged), each a separate row. The penalty is *usually* one value repeated (**507 of 571**) but **64 settlements carry DIFFERING per-facility amounts** (was 72) — so it is **not** a clean broadcast, and neither "take one value" nor "sum" is universally right. Exposing the structure lets the user pick per analysis. **Do NOT sum `PENALTY_AMOUNT` across a settlement's facilities without a broadcast rule.** See `multi_facility_settlement_decision.md` §5 for the FRS-`REGISTRY_ID` crosswalk sub-analysis (552 of 588 settlements span genuinely distinct `REGISTRY_ID`s as last profiled) — not independently re-verified in this pass. |
...
**Shape (as of 2026-07-27; facility count drifts with each live ICIS-AIR refresh, everything else here is
static since the raw pipeline file is a one-time 2026-07-23 manual add, not live-refreshed):**
```
5,872,965 rows | 14 cols | 279,665 facilities × 21 years | 31,796 observed facility-years (0.5%)
...
| **PL3** | **Universe = the same 279,665-facility (as of 2026-07-27) × 2005–2025 rectangle as ds 0/1/2b** (G3/G4), so `pipeline` joins **1:1** to `regulatory.csv.gz` on `(PGM_SYS_ID, YEAR)` (verified: identical key vectors). | Build only over the 20,251 facilities the raw file actually contains. | Consistent with the layer's full-universe convention; as of 2026-07-27 **all 20,251 of 20,251 (100%)** raw-file facilities now match the ICIS universe — the 2 orphans present at last profiling have been absorbed by the 2026-07-26 ICIS-AIR roster growth, so the full rectangle now costs nothing at all. |
...
| placeholder rows (7,193, unchanged — the raw pipeline file is static) structurally absent from every facility-year (no `VIOL_START_DATE` → no year) | ✓ |
...
REGISTRY_ID × PGM_SYS_ACRNM × PGM_SYS_ID × POLLUTANT_NAME`, 10,411,871 rows.
...
| built-dataset pollutant totals (e.g. PM10 $6.02B) are smaller than the raw-file-wide total ($7.04B) — expected, since the dataset is restricted to the ~19.4% ICIS-matched universe (EM3), partially offset by EM2's broadcast | ✓ (explained, not an error) |
