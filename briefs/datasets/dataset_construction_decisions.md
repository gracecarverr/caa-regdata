...
| **O2** | **Strictly raw — NO imputation**, with **one narrow, dated exception added 2026-07-30 (explicit user decision).** Yearly `operating`/`op_status`/`prog_*_active` are carried for 2015–2025 and left **`NA`** for 2005–2014 and any facility-year absent from a snapshot. `WAYBACK_OBSERVED` (1 iff the facility appears in that year's snapshot) is the coverage flag. **The exception:** 2018 (no real snapshot for anyone, see `code/02_cleaning/wayback/README.md`) now gets its `operating` bucket (never `op_status_code`/`op_status_desc`) bridge-imputed for the one case where a facility has a REAL raw 2017 observation AND a REAL raw 2019 observation agreeing on the same bucket (both in-service, or both not) — flagged via the new `OPERATING_IMPUTED` column (1 for imputed rows, 0 otherwise, never `NA`), never silently blended into `WAYBACK_OBSERVED` (which still correctly reads 0 for these rows — no real snapshot exists, the evidence is inferred, not observed). Mismatched 2017→2019 transitions are NOT touched — that timing-unknowable case is `O4`'s existing `W7a` "confirmed non-operating by the next real observation" treatment, and this rule is designed to leave it alone. "Real raw" was chosen over allowing an LOCF-carried neighbor value after checking directly: interior-gap LOCF is vanishingly rare (29 of 2,506,480 non-`NA` facility-years across 2015–2025, 0.001%; only 14 of ~229,000 2017/2019 matching pairs involve an LOCF year) — raw-only costs nothing in coverage and avoids stacking one imputation inside another. | Extend status across pre-2015 / interior gaps using the entry/exit spells (original 2026-07-20 alternative, still not taken for anything but the 2018 exception above). | Mirrors ds 0's zero-vs-NA discipline: don't manufacture certainty Wayback lacks. Spells are provided **separately** (O4) so the user can extend downstream by choice, not by baked-in assumption. *(User decision, 2026-07-20.)* **Why the 2026-07-30 exception:** 2018 is a unique, one-time, fully-documented data artifact (a missing snapshot, not an ordinary per-facility gap already handled by LOCF) — bridging it when both neighbors agree recovers real information at negligible risk, verified before building: it cannot change panel eligibility (`PB2`) since a facility's 2017/2019 real observations already independently qualify it, and it cannot change `wayback_facility_spells.csv.gz` (`O4`) since `op_status_code` is deliberately never fabricated, keeping the imputed row invisible to the exit-transition classifier that depends on `op_status_code`'s non-`NA`-ness as its "real evidence" signal — confirmed empirically (byte-identical `wayback_facility_spells.csv.gz` before/after, `18_wayback_facility_spells.R`), not just by proof. |
