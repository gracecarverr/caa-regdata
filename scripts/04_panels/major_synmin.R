...
#   year, with the full regulatory-activity detail (inspection / violation / enforcement / cert / stack-test
#   breakdowns), interval-based HPV status, formal penalties, and the facility spine attached. The larger
#   regulated sources.
#   in : data/panels/spine.csv.gz  +  data/clean/{inspections,violations,formal_actions,informal_actions,certs,stacktests}.csv.gz
#   out: data/panels/major_synmin.csv.gz   (93 columns: 77 detail + obs_source + 5 spell + 10 year-varying wayback status)
#   Standalone: the panel recipe is spelled out inline (no shared helpers, no config). Copy this file and
#   change the facility filter to make a new panel. The recipe mirrors the CAA_Project panel builder.
#
...
facility_year_panel <- function(facs) {
  ...
  panel |> left_join(facs, by = "PGM_SYS_ID") |>
    ...
    select(all_of(PANEL_COLS)) |> arrange(PGM_SYS_ID, year)
}
...
cat(sprintf("major_synmin panel: %d rows | %d facilities | %d cols | %d-%d\n",
            nrow(panel), n_distinct(panel$PGM_SYS_ID), ncol(panel), min(YEARS), max(YEARS)))
