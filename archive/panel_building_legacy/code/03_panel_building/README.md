# 03_panel_building — facility spine and the sample panels

**Stage inputs:** `data/processed/*.csv.gz` (cleaned assets) + `data/raw/{frs,us_counties}`
**Stage outputs:** `data/panels/{spine,universe,major_synmin,electric}.csv.gz`
...
|-------|------|---------------------|
| `universe`     | 111 | all ever-active facilities (contiguous US + DC) |
| `major_synmin` | 111 | + Major / Synthetic Minor emissions class |
| `electric`     | 111 | + electric utilities (NAICS 2211 / SIC 4911) |
...
Add one `list(name=, filter=)` entry to `PANEL_SPECS` in `03_build_parameters.R`. No new code —
