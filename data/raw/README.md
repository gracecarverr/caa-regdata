...
data) — that code (`code/03_panel_building/01_attainment.R`, its path as of the pre-rename layout) was
already synced to the sibling `CAA_Project` repo on 2026-07-23 and has been dropped from here entirely; see
decision W10 in `archive/panel_building_legacy/briefs/panel/panel_construction_decisions.md`.
...
- **The current ICIS-Air download is a single snapshot** — it has current operating status/class/industry and
  **no facility entry/exit dates**. That is *why* `ICIS_AIR_WAYBACK/` exists: the annual snapshots let the
  ...
  decisions W1–W7 / F7 in `archive/panel_building_legacy/briefs/panel/panel_construction_decisions.md`).
  **2018 has no real snapshot** (W7) — treated as an explicit gap, not inferred.
- **Not every year's snapshot actually lands in Q4.** A 2026-07-29 audit of the Wayback Machine's full
  capture history for this URL (`code/diagnostics/wayback_verify/wayback_q4_repin.R`,
  `output/wayback_verify/q4_repin_summary.csv`) found the Internet Archive simply never crawled it in Q4 of
  2015, 2017, 2019, 2023, or 2025 — 2019 and 2023 have exactly **one** 200-status capture in their entire
  calendar year. Those years are pinned to their best (often only) available capture regardless of month;
  **2024 is the one year that had genuine Q4 alternatives** and was moved from a Sep 26 to a Dec 10 capture.
  See `code/02_cleaning/wayback/README.md` for how this affects the downstream "one snapshot = one panel
  year" interpretation.
