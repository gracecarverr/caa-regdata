...
results, emissions, and (via archived snapshots) how facilities' operating status changes over time. The
backbone is **EPA ICIS-Air** (the current compliance/enforcement system), supplemented by the legacy
**AFS** system and **FRS** for coordinates. The institutional setting — what each system is and why it
matters — is in [`briefs/institutional_overview.md`](../briefs/institutional_overview.md).
