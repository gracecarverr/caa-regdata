...
- **`facilities`** is the *current snapshot* (class, industry, operating status are time-invariant here); the
  ...
  `data/datasets/coordinates.csv.gz` adds the same FRS coordinates + county placement over the full universe.
  Year-varying operating status is reconstructed separately from the Wayback tables (F2, and dataset 1,
  `operating`).
