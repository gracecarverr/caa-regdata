...
| `19_panel_electric_lag_profile.R` | Two cuts of the panel layer `18` doesn't cover: (1) mean regulatory activity and intensity-over-time, electric vs. **other**, netted out within `major_synmin` (`IS_ELECTRIC` tagged by membership in `electric_2015_2025`'s own facility set, not a re-derived NAICS/SIC regex) — `18`'s panel-vs-panel comparison compares electric against electric-plus-everyone-else, this nets "other" out cleanly; (2) regulatory lag (`MEAN_VIOL_TO_EA_LAG_DAYS`, dataset 6) in year *t* vs. future violations (`N_VIOLATIONS`, year *t+1*) — descriptive/correlational only. Added 2026-07-30. | `data/panels/{major_synmin_2015_2025,electric_2015_2025}.csv.gz`, `data/datasets/pipeline.csv.gz` → `output/panel_profile/{electric_vs_other_summary,electric_vs_other_by_year,lag_future_violations,lag_future_violations_bin_summary}.csv` + `output/figures/panels/panel_{electric_vs_other_means,electric_vs_other_over_time,lag_future_violations}.png` |
| `20_panel_extended_profile.R` | Six more electric-vs-other descriptive cuts, brainstormed as follow-ups rather than duplicating `18`/`19`: (1) concentration — Lorenz curves + Gini coefficients for `N_VIOLATIONS`/`N_ENFORCEMENT`/`PENALTY_AMOUNT`, summed per facility over its observed years; (2) cumulative facility exit by calendar year; (3) inspection hit rate (share of inspected facility-years finding ≥1 violation) over time; (4) self-disclosure share of violations over time (`pipeline.csv.gz`); (5) mean enforcement per facility-year by state; (6) emissions volume (`emissions.csv.gz`, criteria-pollutant mass) vs. inspection/enforcement intensity — the one genuinely new cross-dataset join in this line of work. All descriptive/correlational only. Added 2026-07-30. | `data/panels/{major_synmin_2015_2025,electric_2015_2025}.csv.gz`, `data/datasets/{pipeline,emissions}.csv.gz` → `output/panel_profile/{concentration_lorenz,concentration_gini,exit_curve_by_year,inspection_hit_rate_by_year,self_disclosure_share_by_year,state_enforcement_intensity,emissions_enforcement_facility_year,emissions_enforcement_bin_summary}.csv` + `output/figures/panels/panel_{concentration_lorenz,exit_curve,inspection_hit_rate,self_disclosure_share,state_enforcement_intensity,emissions_enforcement}.png` |
| `21_panel_agency_recidivism_profile.R` | Two more cuts, the ones judged most likely to reveal something non-obvious out of `20`'s brainstormed list: (1) EPA vs. state vs. local share of inspections/enforcement over time (`N_INSP_*`/`N_ENF_*`, pooled across major_synmin) — who's actually doing the regulating; (2) repeat-offender persistence — `P(violation at t+k \| violation status at t)`, k=1-3, electric vs. other, against each group's unconditional baseline — tests whether `20`'s concentration finding reflects the same chronic facilities recurring or a different set each year. Both descriptive/correlational only. Added 2026-07-30. | `data/panels/{major_synmin_2015_2025,electric_2015_2025}.csv.gz` → `output/panel_profile/{agency_share_by_year,recidivism_by_lag}.csv` + `output/figures/panels/panel_{agency_share_over_time,recidivism}.png` |
