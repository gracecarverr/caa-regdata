# 04_panel_building — two panels, built from `data/datasets/`
...
**Stage outputs:** `data/panels/{major_synmin_2015_2025,electric_2015_2025}.csv.gz`
...
|---|---|
| `major_synmin_2015_2025` | CONUS, Major/Synthetic-Minor class, **`ACTIVE_BROAD == 1` in at least one year 2015–2025** |
| `electric_2015_2025` | the above **+** NAICS 2211 or SIC 4911 |

**The eligibility rule (revised 2026-07-29, explicit user decision — see `PB2` in
`panel_construction_decisions.md`).** Originally this pipeline required `ACTIVE_BROAD == 1` in **every** one
of the 11 years ("continuously active") — that rule is now retired: requiring full-window continuity drops
exactly the facilities most likely to exit *because of* enforcement/penalty outcomes, biasing an
enforcement/compliance panel toward survivors. The rule is now "active in **at least one** year 2015–2025,"
using `ACTIVE_BROAD` (`data/datasets/operating.csv.gz`, decision `O6` in
`briefs/datasets/dataset_construction_decisions.md`) exactly as before — a facility-year is "active" if
wayback confirms it operating, OR ICIS recorded an event, OR emissions/GHG reporting covers it, **that
specific year**. A facility needs a positive signal in **any one** of the 11 years to pass; only a facility
with zero qualifying years anywhere in the span fails the screen. Every eligible facility still gets a full
11-year rectangle of rows (`build_panel()`'s `expand_grid`) — years outside its actual activity now read as
`OBS_SOURCE == "unobserved"`, which is expected under this rule (see `18_panel_profile.R`'s coverage CSV).

**Why 2015–2025, not 2005–2025:** the eligibility screen is only meaningful over the span `ACTIVE_BROAD`
...
This README explains *what/how*. For *why* — the eligibility rule, the panel scope decision (2 panels, not a
