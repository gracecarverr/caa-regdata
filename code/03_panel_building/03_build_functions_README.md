...
cleaned assets in `data/processed/` + `spine` in `data/panels/`.
...
5. select the canonical column order and arrange by `PGM_SYS_ID, year`.

Same recipe for all three panels (`universe`, `major_synmin`, `electric`) — only the facility filter differs.
...
- `PANEL_COLS` — canonical column order, shared by all three panels.
