...
| `agg_inspections(ids)` | facility-year inspection counts + FCE/PCE + agency splits (`dup==0`) |
| `agg_violations(ids)` | violations + HPV/FRV + program + agency splits (`dup==0`) |
| `agg_enforcement(ids)` | formal+informal pooled: `n_enforcement` (`dup==0`) + `n_enforcement_raw` (all rows) + type/agency splits |
| `agg_certs(ids)` | `n_certs` (`dup==0`) + `n_certs_raw` (all rows) + deviation count |
| `agg_stacktests(ids)` | stack tests + pass/fail (`dup==0`) |
| `attach_hpv_status(panel, ids)` | interval-based `hpv_active` / `hpv_active_1mo` from the HPV spell (union of overlapping spells; open spells day-zero-year-only) |
| `attach_penalty(panel, ids)` | `penalty_amount` = Σ formal penalties (`dup==0`); 0/none → `NA` |
