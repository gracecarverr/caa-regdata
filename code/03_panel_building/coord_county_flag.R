...
flag_coord_county <- function(fac, counties_sf) {
  ...
  if (length(mm)) {
    ...
    claimed <- co5070[match(f$resolved_geoid[mm], as.character(co5070$GEOID)), ]  # the ICIS-claimed polygon per mismatch row
    pts     <- st_transform(st_as_sf(f[mm, ], coords = c("longitude", "latitude"), crs = 4326), 5070)  # the actual coordinate
