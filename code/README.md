...
├── 03_panel_building/  facility spine + sample panels -> data/panels/
├── 04_datasets/        eight purpose-built deliverable datasets -> data/datasets/  (this repo's main product)
...
| 03 panels | `03_panel_building/03_build.R` | `data/panels/*.csv.gz` | spine → universe/major_synmin/electric |
| 04 datasets | `04_datasets/0{1..8}_*.R` | `data/datasets/*.csv.gz` | eight full-universe deliverables (regulatory, operating, hpv_spells, hpv_active, penalties, coordinates, pipeline, emissions) — see `04_datasets/README.md` |
| docs | `diagnostics/build_site.R`, `build_home.R`, `build_databases_page.R`, `06_panel_profile.R` + `build_panels_page.R` | `docs/{raw_data,index,databases,panels}.html` | 4-page site, generated from `data/raw`/briefs/panels; skip with `SKIP_SITE=true` |
...
- **Numbered stages run in dependency order.** `02` needs `01`'s raw; `03` needs `02`'s processed assets;
  `04` needs `02`'s processed assets.
